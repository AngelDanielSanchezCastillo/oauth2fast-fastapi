"""
Integration tests for the optional email-verification grace period (0.4.5).

Covers the configurable behavior of ``POST /auth/token``:

- ``enforce_email_verification`` (default ``False``): master switch. When
  ``False`` the endpoint keeps the 0.4.4 behavior (no verification check).
- ``verification_grace_days`` (default ``None``): when enforcement is on,
  ``None`` rejects unverified users immediately (403); an ``int`` grants a
  grace period counted from ``user.created_at`` (naive datetimes assumed UTC).
"""
...
async def test_default_config_unverified_user_can_login(client: AsyncClient):
    """Config por defecto (enforce=False): el login de usuarios no verificados
    funciona igual que en 0.4.4 (compatibilidad hacia atrás)."""
...
async def test_enforce_without_grace_rejects_unverified_user(client: AsyncClient):
    """enforce=True + grace_days=None: usuario no verificado recibe 403 inmediato."""
...
@pytest.mark.parametrize(
    ("enforce", "grace_days"),
    [(False, None), (True, None), (True, 10)],
)
async def test_verified_user_always_can_login(
    client: AsyncClient, enforce: bool, grace_days: int | None
):
